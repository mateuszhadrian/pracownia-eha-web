...
// Sonda siedziała na kickerze „SIEĆ MISTRZÓW"; po jego usunięciu
// (poprawki klienta) pierwszym [data-rev] płyty jest h2.
test("reveal nagłówka płyty odpala po dojechaniu scrollem", async ({
  page,
  isMobile,
}) => {
  test.skip(!isMobile, "reveale [data-rev] istnieją tylko w układzie mobile");
  await gotoReady(page, PATH);
  const kick = page.locator(".eka-plate-siec .eka-plate-t h2");
